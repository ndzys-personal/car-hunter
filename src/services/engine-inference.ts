...
export interface EngineInference {
  engine: 'N52B25' | 'N52B30' | 'M57' | 'unknown';
  confidence: number;
  evidence: string[];
}
...
export function inferEngine(
  ...
  if (explicit) {
    ...
    return { engine, confidence: 0.98, evidence: [`Kod ${explicit} podany w ogłoszeniu`] };
  }
  ...
  if (listing.fuelType === 'diesel' || /\b(320D|325D|330D|525D|530D)\b/.test(text)) {
    ...
    if (sixCylinderVariant && (sixCylinderCapacity || listing.engineCapacityCc === null)) {
      ...
      const confidence = (listing.engineCapacityCc ?? 0) >= 2900 ? 0.88 : 0.7;
  ...
  if (listing.fuelType === 'petrol' || /\b(325I|330I|525I|530I)\b/.test(text)) {
    const isThreeLiter =
      /330i|530i/i.test(variant) && between(listing.engineCapacityCc, 2900, 3100);
    const isTwoAndHalf =
    ...
    if (isThreeLiter && n52Era) {
      evidence.push('wariant 3.0 benzyna z okresu N52');
      return { engine: 'N52B30', confidence: power >= 250 ? 0.9 : 0.78, evidence };
    }
    if (isTwoAndHalf && n52Era) {
      evidence.push('wariant 2.5 benzyna z okresu N52');
      return { engine: 'N52B25', confidence: power >= 190 ? 0.88 : 0.76, evidence };
    }
